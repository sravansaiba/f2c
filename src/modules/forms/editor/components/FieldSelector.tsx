...
const fieldCategories: any = {
  basic: [
    ...
    { type: "mobile", label: "Phone Input", category: "basic" },
  ],
  intermediate: [
    { type: "checkbox", label: "Checkbox", category: "intermediate" },
    { type: "radio", label: "Radio Buttons", category: "intermediate" },
    { type: "dropdown", label: "Dropdown", category: "intermediate" },
    { type: "country", label: "Country Input", category: "intermediate" },
  ],
  advanced: [
...
export const FieldSelector = ({ onDragStart }: FieldSelectorProps) => {
  ...
  const [activeCategories, setActiveCategories] = useState<Set<string>>(new Set(['basic']));
        ...
        <div className="space-y-2">
          ...
          <div className="mb-2">
            <button
              ...
              className={`flex items-center justify-between w-full px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition duration-200 ease-in-out ${
                isCategoryActive("intermediate") ? "border-b-4 border-black" : ""
              }`}
            >
              <span className="text-sm font-medium text-gray-700">Intermediate Elements</span>
              <span className="text-gray-500">{isCategoryActive("intermediate") ? <ChevronUp size={25}/> : <ChevronDown size={25}/>}</span>
            </button>
            
            {/* Display intermediate Fields */}
            {isCategoryActive("intermediate") && (
              <div className="mt-2 bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                <div className="grid grid-cols-2 gap-3">
                  {fieldCategories.intermediate.map((field: any, index: number) => (
                    <button
                      key={`${field.type}-${index}`}
                      draggable
                      onDragStart={(e) => {
